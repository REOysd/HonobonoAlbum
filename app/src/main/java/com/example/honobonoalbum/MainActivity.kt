...
class MainActivity : ComponentActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if (intent.getBooleanExtra("WAKE_UP_SCREEN", false)) {
            ...
            wakeUpManager.clearWakeUpNotification()
    ...
    private fun testWakeUpDevice() {
        Handler(Looper.getMainLooper()).postDelayed({
            val result = wakeUpManager.forceWakeUpIgnoringLock(vibrate = true)
            Log.d("MainActivity", "forceWakeUpIgnoringLock result: $result")
        }, 10000)
    }
}